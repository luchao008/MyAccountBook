...
.page {
  ...
  padding: 0;
  /*
   * 底部留白：主容器里是「52px 底栏 + 凸起按钮外溢」共 88px。
   * 用 CSS 变量而不是写死，是因为本组件也被账本选择页复用 ——
   * 那一页没有凸起按钮，宿主会把变量改成 52px，否则会多出 36px 空白。
   */
  padding-bottom: calc(var(--view-bottom-gap, 88px) + env(safe-area-inset-bottom));
}
