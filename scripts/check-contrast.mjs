...
section('9. token 文件自洽性（v1.1 现行 · frontend/src/styles/tokens.scss）');
{
  ...
  const TOKEN_CHECKS = [
    ...
    // —— 骨架屏（2026-09-18）：**不是装饰色，是「内容尚未就位」的占位** ——
    //    唯一的验收标准是「在它出现的那几种底色上都能被看见」。
    //    ⚠️ 底色的选择是有后果的：$v11-bg-inset #F5F5F5 压页面底只有 1.03，
    //       而首页区间行 / 报表页多数区块正坐在页面底上（所以它不能拿来当骨架块）。
    //       第一行按惯例写「压白卡」的值，页面底那一档由 §15.2 的双底矩阵覆盖。
    ['$v11-skeleton', '#eaeaea', 1.2, WHITE],
    ['$v11-skeleton-hero', '#ebd4b4', 1.34, '#fdf6ef'],
...
section('13. 字号阶梯自洽（文档 §3.2 ↔ tokens.scss ↔ 各 .vue）');
{
  ...
  //    2026-09-15 更新十五：首页「本月各分类支出排行」加「点击展开 / 收起」
  ...
  //    2026-09-15 更新十八：新增「流水回收站」页（pages/recycle/index.vue）
  ...
  //    2026-09-17 更新二十三：新增「流水导入」独立页（`pages/import/index.vue`，
  //    自绘顶栏 + 三态：选择 / 预览 / 结果）→ 文字 217 → 238，总数 219 → 240。
  //    ⚠️ 该页**没有新增任何图标 `font-size`**（图标一律走 SvgIcon 的 size prop），
  //       所以 `$icon-*` 仍是 2 处 —— 计数变动做了双向归因。
  //    2026-09-17 更新二十四：分类管理页补做拖动排序 —— 底栏普通态由「两格宽条目」改为
  //    「三格图标栅格」（与批量态共用 .batch-act），因此删掉 .batch-entry-title /
  //    .batch-entry-sub / .add-root 三处文字字号；排序态新增 .sort-hint-text 与
  //    .sort-done-text 两处 → 文字 238 → 237，总数 240 → 239。
  //    ⚠️ 这是**净减 1**，已逐条核对：删的 3 处对应选择器在 template 里已无引用，
  //       新增的 2 处都真实承载文案（说明条 + 完成按钮），不是死样式。
  //       拖拽手柄一律走 SvgIcon 的 size prop，所以 `$icon-*` 仍是 2 处。
  //    2026-09-18 更新二十五：首页 / 流水页 / 报表页接入首屏骨架屏，删掉 ReportView
  //    的 `.state` / `.state-text`（「加载中…」那一段被骨架取代）→ 文字 237 → 236，
  //    总数 239 → 238。
  //    ⚠️ 又是一次**净减 1**，同样先核对"少掉的是不是死样式"再改数字：
  //       `grep -n 'state-text|class="state"' ReportView.vue` 已零命中 ——
